...
const resolveThreshold = (candidate?: number) => {
...
const computeDurationMs = (started: bigint) => {
...
export const getAuditionMiddleware = (options?: {
    logger?: { warn: (...args: any[]) => void };
    ...
    return async (
        ctx: { set: (name: string, value: string) => void; method?: string; path?: string; status?: number },
        next: () => Promise<void>,
    ) => {
