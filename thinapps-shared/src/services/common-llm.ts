...
const resolveDeployment = (model: string, cfg: ReturnType<typeof normalizeAzureConfig>) => {
...
const toImageMime = (input: unknown) => {
...
const buildTuningParams = (tuning?: Record<string, unknown>) => {
    if (!tuning) {
        return {};
    }
...
export class LLMManager {
    private cfg?: ReturnType<typeof normalizeAzureConfig>;
    ...
    private getClient(deployment: string) {
    ...
    private async materializeImage(segment: PromptSegment) {
        ...
        if (storage) {
            try {
                ...
                // If blob storage is unavailable or misconfigured, fall back to inline data URLs.
                this.blobStorage = undefined;
            }
        }
    ...
    private async buildMessages(prompt: unknown, system?: string) {
        const segments = flattenPrompt(prompt);
        const textChunks: string[] = [];
        const imageUrls: string[] = [];
        const imagePromises: Array<Promise<string | undefined>> = [];
    ...
    private async streamCompletion(
        ...
        const iterable = client.chat.completions.create(params as any, { signal: controller.signal }) as any;
    ...
    private async singleCompletion(
        ...
        try {
            const result = await client.chat.completions.create(params as any, { signal: controller.signal });
    ...
    async *iterRun(model: string, options: LLMRunOptions): AsyncIterable<string> {
        const cfg = this.getConfig();
