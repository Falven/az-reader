...
const parseFromList = (raw: string) => {
    ...
    return entries.map((entry) => {
        ...
        const countries = countriesChunk
            ? countriesChunk.split(/[;\s,]+/).filter((c) => c.length > 0)
            : undefined;
...
export class ProxyProviderService {
    ...
    async alloc(country: string = 'auto') {
    ...
    *iterAlloc(country: string = 'auto') {
    ...
    private candidatesFor(country?: string) {
        if (!this.pool.length) {
            return [] as ProxyDescriptor[];
        }
        ...
        const targeted = this.pool.filter((proxy) =>
            proxy.countries.length === 0 || proxy.countries.includes(normalized)
        );
