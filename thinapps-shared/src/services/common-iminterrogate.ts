...
const resolveDeploymentName = (requestedModel: string) => {
    ...
    const chosenDeployment = envDeployment !== undefined && envDeployment.trim().length > 0
        ? envDeployment.trim()
        : fallbackDeployment;
...
const loadAzureConfig = (requestedModel: string) => {
...
const imageBufferToDataUrl = (buff: Buffer) => {
    if (buff.length === 0) {
...
const extractText = (content: unknown) => {
    ...
    if (Array.isArray(content)) {
        const pieces = content
            .map((part) => (isTextPart(part) ? part.text : undefined))
...
const buildMessages = (imageDataUrl: string, prompt?: string, system?: string) => {
    const promptText = prompt?.trim();
    const systemText = system?.trim()
        ?? 'You are an image understanding model. Provide a concise description of the supplied image.';
...
export class ImageInterrogationManager {
    ...
    private clientConfig?: z.infer<typeof azureOpenAISchema>;

    private ensureClient(model: string) {
    ...
    async interrogate(
        model: string,
        opts: { image: Buffer; prompt: string; system?: string },
    ) {
