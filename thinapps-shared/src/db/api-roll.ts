...
const toNormalizedTags = (input: unknown) => {
...
export class APIRoll extends FirestoreRecord {
    ...
    static override from(input: any): APIRoll {
        const roll = super.from(input) as APIRoll;

        roll._id = typeof roll._id === 'string' && roll._id.length > 0
            ? roll._id
            : (input._id ?? randomUUID());
        roll.uid = typeof input.uid === 'string' && input.uid.length > 0
            ? input.uid
            : roll.uid;
        roll.tags = toNormalizedTags(input.tags ?? roll.tags);
        roll.status = roll.status ?? API_CALL_STATUS.SUCCESS;
        roll.chargeAmount = typeof roll.chargeAmount === 'number' && Number.isFinite(roll.chargeAmount)
            ? roll.chargeAmount
            : 0;
    ...
    async persist() {
        ...
        return APIRoll.save(this, this._id, true);
    }
}
